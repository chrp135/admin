      ...
      if(that.EXP_IF.test(value)){ //校验日期格式
        if(options.range){
          ...
          lay.each([that.startDate, that.endDate], function(i, item){
            ...
            document.getElementById(that.bindElem.id).previousElementSibling.value = value[0];
            document.getElementById(that.bindElem.id).nextElementSibling.value = value[1];
          });
        } else {
  ...
  Class.prototype.tool = function(btn, type){
    ...
    ,active = {
      ...
      ,clear: function(){
        ...
        that.done(['', {}, {}]);
        document.getElementById(that.bindElem.id).previousElementSibling.value = '';
        document.getElementById(that.bindElem.id).nextElementSibling.value = '';
      }
